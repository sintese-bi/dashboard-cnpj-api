...
DB_CONFIG = f"postgresql://postgres:{db_password_encoded}@159.65.42.225:5432/comercial_BI_Relational"
...
def activities():
    ...
    query = cursor.execute("SELECT DISTINCT  cnae_principal from cnae_cnaes_")
...
def email():
    ...
    query_cnaes = f''' SELECT cnae_principal,cc.data_incio_atividade from cnp_cnpj_ cc where cnae_principal IN {tuple(cnae_names)} 
